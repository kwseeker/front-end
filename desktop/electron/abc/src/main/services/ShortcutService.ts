  ...
  const register = (onlyUniversalShortcuts: boolean = false) => {
    ...
    shortcuts.forEach((shortcut) => {
      try {
        ...
        // 将其他包括快捷键（包括 show_app、mini_window ）的快捷键转换为 electron 的快捷键格式
        const accelerator =
          convertShortcutRecordedByKeyboardEventKeyValueToElectronGlobalShortcutFormat(
            shortcut.shortcut
          )
